...
for i in range(N_t*N_d):
    for j in range(i+1,N_t*N_d):
        if (i in list(range(2*N_t))) and (j not in list(range(2*N_t))):
...
if a > 1:
    ...
    Cdd_print[:,N_t:2*N_t] = Cdd_disp * 1e12
...
filename = "netlist_12awg_10disk_RADDISP_1Diameter_AccInductance_complex_2disks.txt"  # name of the txt file
N_d = 10                         # number of disks
...
with open(filename, "w") as nt:
    ...
    for i in range(N_t*(N_d-1)):
        ...
        if i in list(range(7,12)):
            nt.write("Cdd_n1_%d_%d N%dP%d N%dP%d %0.2fp\n" % (i+1, 2*cur_disk*N_t-i+1, i+1, 1, 2*cur_disk*N_t-i+1, 1, Cdd_n_total[:,11-i]))
        if i in list(range(6,11)):
            nt.write("Cdd_n2_%d_%d N%dP%d N%dP%d %0.2fp\n" % (i+1, 2*cur_disk*N_t-i-1, i+1, 1, 2*cur_disk*N_t-i-1, 1, Cdd_n_total[:,10-i]))
