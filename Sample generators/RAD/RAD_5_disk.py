...
for i in range(N_t*N_d):
    for j in range(i+1,N_t*N_d):
        if (i in list(range(5*N_t))) and (j not in list(range(5*N_t))):
...
if a > 1:
    ...
    Cdd_print[:,4*N_t:5*N_t] = Cdd_disp * 1e12
...
filename = "netlist_12awg_10disk_RADDISP_3Diameter_AccInductance_complex_5disks.txt"  # name of the txt file
N_d = 10                         # number of disks
...
with open(filename, "w") as nt:
    ...
    for i in range(N_t*(N_d-1)):
        ...
        if i in [24,25,26]:
            nt.write("Cdd_n1_%d_%d N%dP%d N%dP%d %0.2fp\n" % (i+1, 2*cur_disk*N_t-i-3, i+1, 1, 2*cur_disk*N_t-i-2, 1, Cdd_n_total[:,i-22]))
        if i in [27,28,29]:
            nt.write("Cdd_n2_%d_%d N%dP%d N%dP%d %0.2fp\n" % (i+1, 2*cur_disk*N_t-i+3, i+1, 1, 2*cur_disk*N_t-i+2, 1, Cdd_n_total[:,i-25]))
