...
for i in range(N_t*N_d):
    for j in range(i+1,N_t*N_d):
        if (i in list(range(N_t))) and (j not in list(range(N_t))):
...
distance = (d_dist**2 + D_wire**2)**0.5
a = distance/D_wire
if a > 1:
...
filename = "netlist_12awg_10disk_RADDISP_1Diameter_AccInductance_complex.txt"  # name of the txt file
N_d = 10                         # number of disks
...
with open(filename, "w") as nt:
    ...
    for i in range(N_t*(N_d-1)):
        ...
        if i in [0,1,2,3,4]:
            nt.write("Cdd_n1_%d_%d N%dP%d N%dP%d %0.2fp\n" % (i+1, 2*cur_disk*N_t-i-1, i+1, 1, 2*cur_disk*N_t-i-1, 1, Cdd_n_total[:,i]))
        if i in [1,2,3,4,5]:
            nt.write("Cdd_n2_%d_%d N%dP%d N%dP%d %0.2fp\n" % (i+1, 2*cur_disk*N_t-i+1, i+1, 1, 2*cur_disk*N_t-i+1, 1, Cdd_n_total[:,i-1]))
