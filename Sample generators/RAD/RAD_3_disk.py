...
d_dist = 1*D_wire                         # distance between centers of conductors for axial displacement
ro = 0     # distance between axes
...
def factor_calc(R1, R2, d, ro):
    # finding max and min of two circles
    A = max(R1, R2)
    a = min(R1, R2)
    # calculating alpha and delta
    alpha = a / A
    delta = d / A
    phi = list(range(0, 181, 15))   # phi angle
    # initializing y variable for Simpson rule
    y = np.zeros(13)
    # loop for estimating the coefficients for the Simpson rule
    for i in range(13):
        # calculation of V
        V = (1 - 2*ro*np.cos(np.deg2rad(phi[i]))/a + (ro/a)**2)**0.5
        # calculation of k_sq
        k_sq = ((1 - alpha*V)**2 + delta**2) / ((1 + alpha*V)**2 + delta**2)
        # numerator of the factor
        num = 1 - ro*np.cos(np.deg2rad(phi[i]))/a
        # calculation of y
        y[i] = f_calc(k_sq) * num / (np.pi * V**1.5)
    ...
    # sum of extremes
    y_extreme = y[0] + y[12]
    # calculation for S factor
    S = (1/36) * (2*y_even + 4*y_odd - y_extreme) * np.pi
    return S
...
y[N_t:] = list(np.asarray(y[N_t:]) + (d_dist - D_wire)) # adding disk separation 
...
for i in range(N_t*N_d):
    for j in range(i+1,N_t*N_d):
        if (i in list(range(3*N_t))) and (j not in list(range(3*N_t))):
            F = factor_calc(R[:,i], R[:,j], d_dist, ro)
...
# Calculation of disk-to-disk capacitances for corresponding disks
distance = (d_dist**2 + ro**2)**0.5
a = distance/D_wire
if a > 1:
    ...
    Cdd_print[:,2*N_t:3*N_t] = Cdd_disp * 1e12

# Calculation of disk-to-disk capacitances for neighbor disks
    
# calculation of the mid-length of the conductors
length_midturns = 2*np.pi*R_mid*1e-3
# calculation of the disk-to-disk capacitances for the 1st section
Cdd_n1 = e_0 * (length_midturns/4) * (e_r * theta / np.log(D_wire/d) + 1/np.tan(theta/2) - 1/np.tan(np.pi/12))
Cdd_n1_piko = Cdd_n1 * 1e12
# calculation of the disk-to-disk capacitances for the 2nd and 4th sections
dist_2 =  (d_dist**2 + ro**2)**0.5
a_2 = dist_2/D_wire
Cdd_n2 = e_0 * (length_midturns/4) * (1/np.sqrt(a_2**2-1)) * (np.arctan(np.sqrt((a_2+1)/(a_2-1))*np.tan(np.pi/4)) - np.arctan(np.sqrt((a_2+1)/(a_2-1))*np.tan(-np.pi/4)))
Cdd_n2_piko = Cdd_n2 * 1e12
Cdd_n4_piko = Cdd_n2_piko
# calculation of the disk-to-disk capacitances for the 3rd section
dist_3 =  (d_dist**2 + (2*ro)**2)**0.5
a_3 = dist_3/D_wire
Cdd_n3 = e_0 * (length_midturns/4) * (1/np.sqrt(a_3**2-1)) * (np.arctan(np.sqrt((a_3+1)/(a_3-1))*np.tan(np.pi/4)) - np.arctan(np.sqrt((a_3+1)/(a_3-1))*np.tan(-np.pi/4)))
Cdd_n3_piko = Cdd_n3 * 1e12
# total disk-to-disk capacitance of neighbor turns
Cdd_n_total = Cdd_n1_piko + Cdd_n2_piko + Cdd_n3_piko + Cdd_n4_piko
...
filename = "netlist_12awg_10disk_RADDISP_1Diameter_AccInductance_complex_3disks.txt"  # name of the txt file
N_d = 10                         # number of disks
...
with open(filename, "w") as nt:
    ...
    for i in range(N_t*(N_d-1)):
        ...
        if i in list(range(12,17)):
            nt.write("Cdd_n1_%d_%d N%dP%d N%dP%d %0.2fp\n" % (i+1, 2*cur_disk*N_t-i-1, i+1, 1, 2*cur_disk*N_t-i-1, 1, Cdd_n_total[:,i-12]))
        if i in list(range(13,18)):
            nt.write("Cdd_n2_%d_%d N%dP%d N%dP%d %0.2fp\n" % (i+1, 2*cur_disk*N_t-i+1, i+1, 1, 2*cur_disk*N_t-i+1, 1, Cdd_n_total[:,i-13]))
